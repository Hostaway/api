## Retrieve reservation logs

Returns the audit trail for a reservation — the same field-level change history that is shown on the reservation details page in the dashboard. Each entry records a single field change, its previous and new values, when it happened, and (when available) the user who made the change.

### Request

```shell
curl -X GET \
  'https://api.hostaway.com/v1/reservations/13/logs?limit=&offset=' \
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://api.hostaway.com/v1/reservations/13/logs?limit=&offset=",
...
xhr.open("GET", "https://api.hostaway.com/v1/reservations/13/logs?limit=&offset=");
...
Request request = new Request.Builder()
  .url("https://api.hostaway.com/v1/reservations/13/logs?limit=&offset=")
  .get()
...
`GET https://api.hostaway.com/v1/reservations/{reservationId}/logs`
